...
class Calculator extends ApiFaker
{
    ...
    private function getClient(string $iin)
    {
        ...
        if ($yearOld < 25 && $this->getExperience($this->experinceType) < 2) {
            $ageAndExperience = 1.10;
        }
        if (($yearOld < 25 && $this->getExperience($this->experinceType) > 2) || ($yearOld >= 25 && $this->getExperience($this->experinceType) < 2) ) {
    ...
    {
        return (self::MRP * 1.9) * $this->getClient($this->iin) * $this->getVehicle($this->vehicleNumber);
    }

    public function getExperience($i): int
    {
        if ($this->experinceType === 'Менее 2 лет' || $this->experinceType === '2 жылдан кем') {
            $experience  = 1 ;
        }
        else {
            $experience = 3;
